...
var entity_fgd := preload("res://addons/qodot/game_definitions/fgd/teamkill_fgd.tres")
			...
			if classname in entity_definitions:
				...
				if entity_definition is QodotFGDSolidClass:
					...
					if entity_definition.scene_file:
						#print(entity_definition.scene_file)
						node = entity_definition.scene_file.instance()
